...
> **버튼 위계 원칙**: Primary(검정 fill)는 **메인 CTA "성과 문장으로 바꾸기" 한 곳에만** 사용.
> 그 외 모든 액션(저장 · 로그인 · 내보내기 · 의견 남기기 · 닫기 등)은 outline 톤(Secondary) 으로 통일.

| 종류 | 스타일 |
|------|--------|
| **Primary** (`.btn-primary`) | `--ink-900` 배경, 흰 텍스트, radius 12, hover 시 orange 전환. **메인 CTA 한 곳에만 사용** |
| **Secondary** (`.btn-secondary`) | 투명 배경, `1px solid var(--ink-300)` 보더, ink-900 텍스트, hover 시 보더 ink-900 + 옅은 ink-50 배경. 저장·로그인·내보내기·의견 남기기 등 일반 액션 |
| **Ghost** (`.btn-ghost`) | `--ink-100` 배경, ink-700 텍스트, hover 시 ink-200. 보조적인 닫기/뒤로가기 |
| **Icon button** (`.icon-btn`) | 32px 사각형, ink-100 배경, ink-700 아이콘. danger variant 는 hover 시 orange-soft 배경 |
| **CTA action** (변환 카드 액션) | 작은 알약, secondary 톤(outline) 통일 — 복사 / 저장 / 다시 생성 모두 동일 위계 |
